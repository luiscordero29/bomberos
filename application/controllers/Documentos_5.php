...
class Documentos_5 extends CI_Controller {

	/**
	 * Documentos_5
	 **/

	public $controller = "documentos_5";
	...
	public function __construct()
	{
		...
		$this->load->model('Documentos_5_model','',TRUE);
	...
	public function index($table_page=null,$id=null,$search=null)
	{
		$data['breadcrumbs'] = 
			array(
	            ...
	            '<i class="fa fa-fw fa-table"></i> Incendio Forestal y Vegetación'			=> '',        	
	        );
		...
		$data['table'] = $this->Documentos_5_model->table($table_page*$table_limit,$table_limit,$data['search']);
		$data['table_rows'] = $this->Documentos_5_model->table_rows($data['search']);
	...
	public function create()
	{
		$data['breadcrumbs'] = 
			array(
	            '<i class="fa fa-fw fa-home"></i> Tablero Principal'			=> 'dashboard/index',
	            '<i class="fa fa-fw fa-table"></i> Incendio Forestal y Vegetación'			=> $this->controller.'/index',
		...
		$this->form_validation->set_rules('terreno_direccion', 'Dirección', 'trim|required');
		$this->form_validation->set_rules('terreno_tipo', 'Tipo', 'trim|required');
		$this->form_validation->set_rules('terreno_propietario', 'Propietario', 'trim');
		$this->form_validation->set_rules('terreno_vegetacion', 'Tipo de Vegetación', 'trim');
		$this->form_validation->set_rules('terreno_topografia', 'Topografía del Terreno', 'trim');
		$this->form_validation->set_rules('terreno_area', 'Área afectada', 'trim');
		$this->form_validation->set_rules('terreno_cortafuego', 'Corta Fuego', 'trim');
		$this->form_validation->set_rules('terreno_largo', 'Largo', 'trim');
		$this->form_validation->set_rules('terreno_ancho', 'Ancho', 'trim');
		$this->form_validation->set_rules('terreno_natural', 'Natural', 'trim');
		$this->form_validation->set_rules('terreno_artificial', 'Artificial', 'trim');
		$this->form_validation->set_rules('terreno_oc_agua', 'Agua', 'trim|required');
		$this->form_validation->set_rules('terreno_oc_agua_cantidad', 'Cantidad de Agua', 'trim|required|numeric');
		$this->form_validation->set_rules('terreno_oc_espuma', 'Espuma', 'trim|required');
		$this->form_validation->set_rules('terreno_oc_espuma_cantidad', 'Cantidad de Espuma', 'trim|required|numeric');
		...
		$data['table_estados'] = $this->Documentos_5_model->table_estados();
		...
		{				
			...
			$this->Documentos_5_model->create();
	...
	{			
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			$data['row'] = $this->Documentos_5_model->read($id);
			if(empty($data['row']))
			{
				$data['alert']['danger'] = 
						array( 
			...
			$this->Documentos_5_model->update();
			$data['alert']['success'] = 
				array( 
					'Guardado Exitosamente',				
				);
	...
	public function delete($id=false)
	{
		...
		$check = $this->Documentos_5_model->delete($id);
		if($check)
		{
		    $data['alert']['success'] = 
			array( 
    ...
    {	     	
	    ...
	    $rows = $this->Documentos_5_model->table_municipios($id_estado);
    ...
    {	     	
	    ...
	    $rows = $this->Documentos_5_model->table_parroquias($id_municipio);
