...
class Documentos_1 extends CI_Controller {

	/**
	 * Documentos_1
	 **/

	public $controller = "documentos_1";
	...
	public function __construct()
	{
		...
		$this->load->model('Documentos_1_model','',TRUE);
	...
	public function index($table_page=null,$id=null,$search=null)
	{
		$data['breadcrumbs'] = 
			array(
	            ...
	            '<i class="fa fa-fw fa-table"></i> Accidentes de Trancitos'			=> '',        	
	        );
		...
		$data['table'] = $this->Documentos_1_model->table($table_page*$table_limit,$table_limit,$data['search']);
		$data['table_rows'] = $this->Documentos_1_model->table_rows($data['search']);
	...
	public function create()
	{
		$data['breadcrumbs'] = 
			array(
	            '<i class="fa fa-fw fa-home"></i> Tablero Principal'			=> 'dashboard/index',
	            '<i class="fa fa-fw fa-table"></i> Accidentes de Trancitos'			=> $this->controller.'/index',
		...
		$this->form_validation->set_rules('sitio_suceso', 'Sitio del Suceso', 'trim|required');
		$this->form_validation->set_rules('tipo_suceso', 'Tipo de Suceso', 'trim|required');
		$this->form_validation->set_rules('n_lesionados', 'Número de Lesionados', 'trim|required|is_natural');
		$this->form_validation->set_rules('n_fallecidos', 'Número de Fallecidos', 'trim|required|is_natural');
		...
		$this->form_validation->set_rules('uvir_tipo', 'Tipo', 'trim');
		$this->form_validation->set_rules('uamb', 'Unidad/Ambulancia', 'trim');
		$this->form_validation->set_rules('uamb_cp', 'Conducida Por', 'trim');
		$this->form_validation->set_rules('uamb_am', 'Al Mando', 'trim');
		...
		$data['table_estados'] = $this->Documentos_1_model->table_estados();
		...
		{				
			...
			$this->Documentos_1_model->create();
	...
	{			
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			$data['row'] = $this->Documentos_1_model->read($id);
			if(empty($data['row']))
			{
				$data['alert']['danger'] = 
						array( 
			...
			$this->Documentos_1_model->update();
			$data['alert']['success'] = 
				array( 
					'Guardado Exitosamente',				
				);
	...
	public function delete($id=false)
	{
		...
		$check = $this->Documentos_1_model->delete($id);
		if($check)
		{
		    $data['alert']['success'] = 
			array( 
    ...
    {	     	
	    ...
	    $rows = $this->Documentos_1_model->table_municipios($id_estado);
    ...
    {	     	
	    ...
	    $rows = $this->Documentos_1_model->table_parroquias($id_municipio);
