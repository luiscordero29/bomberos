...
class Documentos_7 extends CI_Controller {

	/**
	 * Documentos_7
	 **/

	public $controller = "documentos_7";
	...
	public function __construct()
	{
		...
		$this->load->model('Documentos_7_model','',TRUE);
	...
	public function index($table_page=null,$id=null,$search=null)
	{
		$data['breadcrumbs'] = 
			array(
	            ...
	            '<i class="fa fa-fw fa-table"></i> Guardia de Seguridad Prevención'			=> '',        	
	        );
		...
		$data['table'] = $this->Documentos_7_model->table($table_page*$table_limit,$table_limit,$data['search']);
		$data['table_rows'] = $this->Documentos_7_model->table_rows($data['search']);
	...
	public function create()
	{
		$data['breadcrumbs'] = 
			array(
	            '<i class="fa fa-fw fa-home"></i> Tablero Principal'			=> 'dashboard/index',
	            '<i class="fa fa-fw fa-table"></i> Guardia de Seguridad Prevención'			=> $this->controller.'/index',
		...
		$this->form_validation->set_rules('servicio_tipo', 'Tipo de Servicio', 'trim|required');
		$this->form_validation->set_rules('servicio_motivacion', 'Motivado a', 'trim|required');
		$this->form_validation->set_rules('servicio_direccion', 'Dirección', 'trim|required');
		$this->form_validation->set_rules('servicio_persona', 'Nombre y Apellido', 'trim|required');
		$this->form_validation->set_rules('servicio_persona_edad', 'Edad', 'trim|required|is_natural_no_zero');
		$this->form_validation->set_rules('servicio_persona_identidad', 'C.I', 'trim|required|is_natural_no_zero|min_length[6]|max_length[10]');
		$this->form_validation->set_rules('servicio_persona_telefono', 'Tel&eacute;fono', 'trim|is_natural_no_zero');
		$this->form_validation->set_rules('servicio_materiales', 'Materiales Utilizados', 'trim');
		...
		$data['table_estados'] = $this->Documentos_7_model->table_estados();
		...
		{				
			...
			$this->Documentos_7_model->create();
	...
	{			
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			$data['row'] = $this->Documentos_7_model->read($id);
			if(empty($data['row']))
			{
				$data['alert']['danger'] = 
						array( 
			...
			$this->Documentos_7_model->update();
			$data['alert']['success'] = 
				array( 
					'Guardado Exitosamente',				
				);
	...
	public function delete($id=false)
	{
		...
		$check = $this->Documentos_7_model->delete($id);
		if($check)
		{
		    $data['alert']['success'] = 
			array( 
    ...
    {	     	
	    ...
	    $rows = $this->Documentos_7_model->table_municipios($id_estado);
    ...
    {	     	
	    ...
	    $rows = $this->Documentos_7_model->table_parroquias($id_municipio);
